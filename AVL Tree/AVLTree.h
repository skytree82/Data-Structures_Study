...
public:
	...
	BTree<T>* AVLTSearch(T target);
	void PrintData() {
		InorTrav(avlt, Print); // 중위순회 데이터 출력 (오름차순)
	}
};

template <class T>
void AVLTree<T>::AVLTInsert(T data) {
	if (avlt == NULL) {
...
template <class T>
BTree<T>* AVLTree<T>::AVLTRemove(T target) {
...
template <class T>
BTree<T>* AVLTree<T>::AVLTSearch(T target) {
	BTree<T>* curNode = avlt;
	T curData;
...
BTree<T>* LRRotate(BTree<T>* bst) {
	...
	pNode->ChangeLeftSubTree(RRRotate(cNode));
...
BTree<T>* RLRotate(BTree<T>* bst) {
	...
	pNode->ChangeRightSubTree(LLRotate(cNode));
